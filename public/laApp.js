...
document
	.querySelector('.nuevaPregunta button')
	.addEventListener('click', llamandoUnaPregunta);

function llamandoUnaPregunta() {
	let nombreCategoriaAEnviar = document.querySelector(
		'.categoriaActual h2'
	).innerText;
...
function paseleSrPregunta(todaLaCategoria) {
	document.querySelector('.textoPregunta').innerText =
		todaLaCategoria.preguntas;
	document.querySelector('.respuestaCorrecta textarea').value =
		todaLaCategoria.respuestas;
	...
	document
		.querySelector('.respuestas button')
		.addEventListener('click', revelarRespuesta);
	document.querySelector('.preguntaInfo .numeroId p').innerText =
		todaLaCategoria.id;
}
...
function cambiandoMetaPreguntasConCuadrito(num) {
	document.querySelector(`.solo${metaPreguntas}`).style.backgroundColor =
		'white';
	contestadasMal = 0;
